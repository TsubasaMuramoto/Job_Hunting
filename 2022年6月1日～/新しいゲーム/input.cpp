...
bool CInput::PressAnyAction(const ACTION &action)
{
	...
	case ACTION_UP:		// 上昇など
		return pKey->GetPress(DIK_UP) || pXPad->GetButtonTrigger(XINPUT_GAMEPAD_DPAD_UP) || pDPad->GetGamepad().lY <= -DINPUT_STICK_RANGE ||
			pKey->GetPress(DIK_W) || pXPad->GetGamePad()->m_state.Gamepad.sThumbLY >= XINPUT_GAMEPAD_LEFT_THUMB_DEADZONE;

	case ACTION_DOWN:	// 下降など
		return pKey->GetPress(DIK_DOWN) || pXPad->GetButtonTrigger(XINPUT_GAMEPAD_DPAD_DOWN) || pDPad->GetGamepad().lY >= DINPUT_STICK_RANGE ||
	...
	case ACTION_LEFT:	// 左行動
		return (pKey->GetPress(DIK_A) || pXPad->GetButtonTrigger(XINPUT_GAMEPAD_DPAD_LEFT) || pDPad->GetGamepad().lX <= -DINPUT_STICK_RANGE) ||
				pKey->GetPress(DIK_LEFT) || pXPad->GetGamePad()->m_state.Gamepad.sThumbLX <= -XINPUT_GAMEPAD_LEFT_THUMB_DEADZONE;

	case ACTION_RIGHT:	// 右行動
		return (pKey->GetPress(DIK_D) || pXPad->GetButtonTrigger(XINPUT_GAMEPAD_DPAD_RIGHT) || pDPad->GetGamepad().lX >= DINPUT_STICK_RANGE) ||
